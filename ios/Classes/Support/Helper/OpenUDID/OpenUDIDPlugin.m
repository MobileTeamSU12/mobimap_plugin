...
@interface OpenUDIDPlugin (Private)
...
//
+ (NSString*) value {
    return [OpenUDIDPlugin valueWithError:nil];
}
...
//        [appUID autorelease];
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [OpenUDIDPlugin _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [OpenUDIDPlugin _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [OpenUDIDPlugin _setDict:localDict forPasteboard:slotPB];
    }
